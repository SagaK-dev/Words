...
import { createDeck, loadData, saveData, validateData } from './src/storage.js';
...
function renderDeck(id) {
  ...
  app.querySelector('#shareDeck').onclick=async()=>{ const value=await encodeDeck({...deck,id:'shared'}); const url=`${location.origin}${location.pathname}#share/${value}`; await navigator.clipboard.writeText(url); toast('共有リンクをコピーしました'); };
  wireNav();
}
...
function renderSettings() {
  ...
  <div class="setting-row"><div><strong>キーボード</strong><p>× / △ / ○ のキー。</p></div><div class="form-row"><input id="keyAgain" value="${h(data.settings.keys.again)}" maxlength="1"><input id="keyHard" value="${h(data.settings.keys.hard)}" maxlength="1"><input id="keyGood" value="${h(data.settings.keys.good)}" maxlength="1"></div></div>
  <div class="setting-row"><div><strong>学習データ</strong><p>JSONで書き出し・復元。</p></div><div class="actions"><button class="btn" id="exportData">書き出す</button><label class="btn">読み込む<input id="importData" type="file" accept="application/json" hidden></label></div></div>
  ...
  const saveSettings=()=>{data.settings.fontScale=Number(app.querySelector('#fontScale').value);data.settings.revealMs=Number(app.querySelector('#revealMs').value);data.settings.retryMode=app.querySelector('#retryMode').value;data.settings.keys={again:app.querySelector('#keyAgain').value||'1',hard:app.querySelector('#keyHard').value||'2',good:app.querySelector('#keyGood').value||'3',reveal:' '};commit();renderSettings()};
  ...
  app.querySelector('#importData').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const parsed=validateData(JSON.parse(await file.text()));if(!parsed)throw new Error('Invalid backup');data=parsed;commit();toast('復元しました');renderSettings()}catch{toast('バックアップを読み込めませんでした')}};
...
function startSession(deck, mode='study', count='all') {
  ...
  activeSession={deckId:deck.id,deckName:deck.name,mode,queue,index:0,results:[],startedAt:Date.now(),phase:'prompt',shownAt:Date.now(),rapid:false,revealCount:0};nav('study');
}

function renderStudy() {
  if(!activeSession)return nav('home'); const s=activeSession;if(s.index>=s.queue.length){finishSession();return}
  const card=s.queue[s.index];s.shownAt=Date.now();s.phase='prompt';
...
function showAnswer() { const s=activeSession;if(!s||s.phase!=='prompt')return;clearReveal();s.phase='answer';app.querySelector('#prompt').textContent=s.queue[s.index].front;app.querySelector('#answer').hidden=false;app.querySelector('#note')&&(app.querySelector('#note').hidden=false);app.querySelector('#studyActions').hidden=true;app.querySelector('#rating').hidden=false; }
function startRapidReveal(text) { const s=activeSession;if(!s)return;clearReveal();s.rapid=true;s.revealCount=0;const prompt=app.querySelector('#prompt');prompt.textContent='';revealTimer=setInterval(()=>{s.revealCount+=1;prompt.textContent=text.slice(0,s.revealCount);if(s.revealCount>=text.length)clearReveal()},data.settings.revealMs); }
function clearReveal(){if(revealTimer){clearInterval(revealTimer);revealTimer=null}}
function rateCurrent(rating) { const s=activeSession,card=s.queue[s.index],responseMs=Math.max(100,Date.now()-s.shownAt);if(s.mode!=='test')data.progress[card.id]=scheduleReview(data.progress[card.id]||defaultProgress(card.id),rating,responseMs);s.results.push({cardId:card.id,rating,responseMs});const retryMode=data.settings.retryMode;if(rating===0&&retryMode==='same'&&s.mode!=='test')s.queue.splice(Math.min(s.index+2,s.queue.length),0,card);commit();if(rating===0&&retryMode==='multipleChoice'&&s.mode!=='test')return showRetryChoice(card);s.index+=1;renderStudy(); }

function showRetryChoice(card){const s=activeSession;const pool=s.queue.filter(c=>c.id!==card.id).map(c=>c.back).filter(v=>v!==card.back);const options=[card.back,...shuffle([...new Set(pool)]).slice(0,3)];while(options.length<4)options.push(`— ${options.length+1} —`);shuffle(options);app.querySelector('#rating').hidden=true;const choices=app.querySelector('#choices');choices.innerHTML=`<p class="muted">すぐに4択で覚え直し</p>${options.map(v=>`<button data-value="${h(v)}">${h(v)}</button>`).join('')}`;choices.querySelectorAll('button').forEach(btn=>btn.onclick=()=>{const ok=btn.dataset.value===card.back;btn.classList.add(ok?'correct':'wrong');if(!ok)[...choices.querySelectorAll('button')].find(b=>b.dataset.value===card.back)?.classList.add('correct');setTimeout(()=>{s.index+=1;renderStudy()},450)});}

function finishSession(){const s=activeSession;const reviewed=s.results.length,good=s.results.filter(r=>r.rating===2).length,again=s.results.filter(r=>r.rating===0).length,avg=reviewed?Math.round(s.results.reduce((n,r)=>n+r.responseMs,0)/reviewed):0;data.sessions.push({id:crypto.randomUUID(),deckId:s.deckId,mode:s.mode,startedAt:s.startedAt,endedAt:Date.now(),reviewed,good,again,avgResponseMs:avg});data.sessions=data.sessions.slice(-500);commit();activeSession={...s,summary:{reviewed,good,again,avg}};nav('summary');}
...
async function openSyncModal(){openModal(`<div class="section-head"><div><span class="eyebrow">OPTIONAL CLOUD SYNC</span><h2>同期設定</h2></div><button class="btn close-modal">×</button></div><p class="muted">Cloudflare D1を設定した環境で利用できます。同じ「同期コード」と「パスフレーズ」を別端末に入力すると同期できます。問題数や同期回数のアプリ内上限はありません。</p><div class="field"><label>同期コード（12文字以上）</label><input id="syncCode" value="${h(data.sync.code||randomCode())}"></div><div class="field"><label>パスフレーズ</label><input id="syncPass" type="password" autocomplete="new-password"></div><div class="actions"><button class="btn primary" id="syncPush">この端末 → クラウド</button><button class="btn" id="syncPull">クラウド → この端末</button></div>`,root=>{root.querySelector('#syncPush').onclick=async()=>{try{const code=root.querySelector('#syncCode').value,pass=root.querySelector('#syncPass').value;if(pass.length<8)throw new Error('パスフレーズは8文字以上にしてください');const payload=await encryptJson(data,pass);const res=await fetch('./api/sync',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code,payload})});const body=await res.json();if(!res.ok)throw new Error(body.error||'同期失敗');data.sync={code,updatedAt:body.updatedAt};commit();toast('クラウドへ保存しました')}catch(e){toast(e.message)}};root.querySelector('#syncPull').onclick=async()=>{try{const code=root.querySelector('#syncCode').value,pass=root.querySelector('#syncPass').value;const res=await fetch(`./api/sync?code=${encodeURIComponent(code)}`);const body=await res.json();if(!res.ok)throw new Error(body.error||'取得失敗');const restored=validateData(await decryptJson(body.payload,pass));if(!restored)throw new Error('同期データが不正です');data=restored;data.sync={code,updatedAt:body.updatedAt};commit();closeModal();render();toast('クラウドから復元しました')}catch(e){toast(e.message)}};});}
...
async function decryptJson(payload,pass){const [s,i,c]=payload.split('.').map(fromB64);const key=await deriveKey(pass,s,['decrypt']);const plain=await crypto.subtle.decrypt({name:'AES-GCM',iv:i},key,c);return JSON.parse(new TextDecoder().decode(plain))}
async function deriveKey(pass,salt,usage){const material=await crypto.subtle.importKey('raw',new TextEncoder().encode(pass),'PBKDF2',false,['deriveKey']);return crypto.subtle.deriveKey({name:'PBKDF2',salt,iterations:150000,hash:'SHA-256'},material,{name:'AES-GCM',length:256},false,usage)}
function toB64(bytes){let s='';for(const b of bytes)s+=String.fromCharCode(b);return btoa(s)}function fromB64(s){return Uint8Array.from(atob(s),c=>c.charCodeAt(0))}
...
window.addEventListener('keydown',e=>{if(route()!=='study'||!activeSession)return;const keys=data.settings.keys;if(activeSession.phase==='prompt'&&e.key===' '){e.preventDefault();showAnswer()}else if(activeSession.phase==='answer'){if(e.key===keys.again)rateCurrent(0);if(e.key===keys.hard)rateCurrent(1);if(e.key===keys.good)rateCurrent(2)}});
